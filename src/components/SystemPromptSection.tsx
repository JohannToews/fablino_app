...
const SystemPromptSection = ({ language }: SystemPromptSectionProps) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ══════════ 0b. FSE2 Writer Prompt (v3) — story_writer_prompt_version ══════════ */}
      <Collapsible open={openSections.writerPromptVersion} onOpenChange={() => toggleSection('writerPromptVersion')}>
        <Card className="border-2 border-cyan-500/50 bg-cyan-50/30 dark:bg-cyan-950/20">
          <CollapsibleTrigger asChild>
            <CardHeader className="pb-3 cursor-pointer hover:bg-muted/50 transition-colors">
              <CardTitle className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-lg">
                  {openSections.writerPromptVersion ? <ChevronDown className="h-5 w-5" /> : <ChevronRight className="h-5 w-5" />}
                  <Rocket className="h-5 w-5 text-cyan-500" />
                  FSE2 Writer Prompt (v3)
                  <span className="ml-2 px-2 py-0.5 text-xs font-semibold bg-cyan-500 text-white rounded-full">
                    🚀 FSE2
          ...
          <CollapsibleContent>
            <CardContent className="space-y-4 pt-0">
              <div className="p-3 bg-cyan-100/50 dark:bg-cyan-900/30 rounded-md border border-cyan-300/50">
                <p className="text-sm text-cyan-800 dark:text-cyan-200">
                  🚀 FSE2 Writer — der aktive Writer-Prompt für die FSE2-Pipeline
                </p>
                <p className="text-xs text-cyan-700 dark:text-cyan-300 mt-1 font-mono">
                  DB Key: story_writer_prompt_version
                </p>
              </div>
              ...
              {isLoading ? (
                ...
                <>
                  <Textarea
                    value={writerPromptVersion}
                    onChange={(e) => setWriterPromptVersion(e.target.value)}
                    className="min-h-[350px] text-sm font-mono leading-relaxed"
                    placeholder="FSE2 Writer system prompt (story_writer_prompt_version)..."
                  />
                  <div className="flex items-center gap-3">
                    <Button
                      onClick={async () => {
                        setIsSavingWriterPromptVersion(true);
                        try {
                          const { error } = await invokeEdgeFunction("manage-users", {
                            action: "updateSystemPrompt",
                            promptKey: "story_writer_prompt_version",
                            promptValue: writerPromptVersion,
                          });
                          if (error) {
                            toast.error(language === 'de' ? "Fehler beim Speichern" : "Error saving");
                          } else {
                            toast.success(language === 'de' ? "FSE2 Writer Prompt (v3) gespeichert" : "FSE2 Writer Prompt (v3) saved");
                          }
                        } catch (err) {
                          console.error("Error:", err);
                          toast.error(language === 'de' ? "Fehler beim Speichern" : "Error saving");
                        } finally {
                          setIsSavingWriterPromptVersion(false);
                        }
                      }}
                      disabled={isSavingWriterPromptVersion}
                      className="btn-primary-kid"
                    >
                      {isSavingWriterPromptVersion ? (
                        <>
                          <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                          {language === 'de' ? 'Speichern...' : 'Saving...'}
                        </>

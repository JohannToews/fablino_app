# Emotion-Flow-Engine — Content Spec: Emotion Blueprints

> **Zweck**: Seed-Daten für `emotion_blueprints` Tabelle.  
> **Format**: Jeder Blueprint enthält alle Felder, ready für SQL INSERT.  
> **Erstellt**: 2026-02-21  
> **Basis**: Phase 1 Blueprint + Audit v2

---

## Hinweise für Implementation

- `labels` und `descriptions` → 7 Sprachen (de, fr, en, es, nl, it, bs). Hier nur DE + EN gezeigt. Restliche Sprachen werden via LLM in Phase 3 generiert.
- `arc_by_age` → 3 Varianten (6-7, 8-9, 10-11). `12+` nutzt die 10-11 Variante.
- `arc_prompt` ist der Text der ans LLM geht — ENGLISCH, konkret, narrativ.
- `compatible_themes` referenziert die 4 Subtype-Kategorien: `magic_fantasy`, `adventure_action`, `real_life`, `surprise`
- `compatible_learning_themes` referenziert Keys aus `learning_themes` Tabelle

---

## Kategorie: Growth (Wachstum)

### Blueprint 1: `overconfidence_and_fall`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Übermut und Fall", "en": "Overconfidence and Fall"}` |
| **descriptions** | `{"de": "Kind überschätzt sich, scheitert, lernt Hilfe anzunehmen", "en": "Child overestimates themselves, fails, learns to accept help"}` |
| **category** | `growth` |
| **min_intensity** | `medium` |
| **compatible_themes** | `{magic_fantasy, adventure_action, real_life, surprise}` |
| **ideal_age_groups** | `{8-9, 10-11}` |
| **compatible_learning_themes** | `{frustration_tolerance, teamwork}` |
| **tone_guidance** | "Mix humor into the failure scene — the failure should be partly FUNNY, exaggerated, physical, not tragic. The shame should be brief, not crushing. The ending warmth should sneak up." |
| **tension_curve** | "medium → high → crash → low → medium → warm" |
| **surprise_moment** | "The help comes from the person the protagonist dismissed at the beginning. The friend's 'useless' hobby or skill turns out to be the key." |
| **ending_feeling** | "Quiet pride. Not a triumphant fist-pump. A small smile and the thought: maybe I don't have to do everything alone." |
...
### Blueprint 2: `fear_to_courage`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Von Angst zu Mut", "en": "Fear to Courage"}` |
| **descriptions** | `{"de": "Kind überwindet eine konkrete Angst Schritt für Schritt", "en": "Child overcomes a specific fear step by step"}` |
| **category** | `growth` |
| **min_intensity** | `medium` |
| **compatible_themes** | `{magic_fantasy, adventure_action, real_life, surprise}` |
| **ideal_age_groups** | `{6-7, 8-9, 10-11}` |
| **compatible_learning_themes** | `{self_confidence, emotional_regulation}` |
| **tone_guidance** | "The fear must feel REAL and specific — not abstract 'being scared' but concrete physical sensations (stomach knots, sweaty palms, wobbly knees). The courage is NOT fearlessness — it's acting DESPITE fear." |
| **tension_curve** | "high → higher → peak → release → warm" |
| **surprise_moment** | "The scary thing turns out to be different from what the protagonist imagined. The reality is manageable — it was the imagination that was terrifying." |
| **ending_feeling** | "I was scared, but I did it anyway. Not fearless — brave." |
...
### Blueprint 3: `failure_is_learning`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Scheitern ist Lernen", "en": "Failure is Learning"}` |
| **descriptions** | `{"de": "Kind scheitert, wird frustriert, findet einen neuen Weg", "en": "Child fails, gets frustrated, finds a new approach"}` |
| **category** | `growth` |
| **min_intensity** | `medium` |
| **compatible_themes** | `{adventure_action, real_life, surprise}` |
| **ideal_age_groups** | `{6-7, 8-9, 10-11}` |
| **compatible_learning_themes** | `{frustration_tolerance, perseverance}` |
| **tone_guidance** | "The frustration should feel authentic — not cute, not brief. Let the child SIT in the frustration for a moment. The new approach should come from observation or curiosity, not from an adult's advice." |
| **tension_curve** | "high → crash → low → curious → rising → success" |
| **surprise_moment** | "The solution comes from an unexpected angle — a 'mistake' from an earlier attempt turns out to be useful, or a completely different approach works." |
| **ending_feeling** | "Satisfaction and a spark of curiosity: 'What else could I try?'" |
...
### Blueprint 4: `finding_your_voice`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Die eigene Stimme finden", "en": "Finding Your Voice"}` |
| **descriptions** | `{"de": "Kind überwindet Schüchternheit und verschafft sich Gehör", "en": "Child overcomes shyness and makes themselves heard"}` |
| **category** | `growth` |
| **min_intensity** | `deep` |
| **compatible_themes** | `{real_life, magic_fantasy, surprise}` |
| **ideal_age_groups** | `{8-9, 10-11}` |
| **compatible_learning_themes** | `{self_confidence, conflict_resolution}` |
| **tone_guidance** | "The silence should feel heavy. The moment of speaking up should feel like the loudest thing in the world — even if it's actually quiet. Don't make the protagonist suddenly confident. The voice shakes. The words come out wrong. But they COME OUT." |
| **tension_curve** | "low → frustrating → building → explosive → warm" |
| **surprise_moment** | "When the protagonist finally speaks, the reaction is not what they feared. People were waiting to hear from them." |
| **ending_feeling** | "My voice matters. It was always there — I just hadn't used it yet." |
...
## Kategorie: Social (Miteinander)

### Blueprint 5: `standing_up_for_others`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Für andere einstehen", "en": "Standing Up for Others"}` |
| **descriptions** | `{"de": "Kind beobachtet Ungerechtigkeit und findet Mut einzugreifen", "en": "Child witnesses injustice and finds courage to intervene"}` |
| **category** | `social` |
| **min_intensity** | `medium` |
| **compatible_themes** | `{real_life, adventure_action, magic_fantasy}` |
| **ideal_age_groups** | `{8-9, 10-11}` |
| **compatible_learning_themes** | `{fairness, empathy, conflict_resolution}` |
| **tone_guidance** | "The injustice must feel WRONG to the reader — not just told, but shown. The protagonist's fear of intervening must be understandable. Standing up should cost something." |
| **tension_curve** | "witnessing → uncomfortable → tense → brave → relief + belonging" |
| **surprise_moment** | "Standing up doesn't fix everything instantly. But it changes the dynamic — others join in, or the unfairness becomes visible to everyone." |
| **ending_feeling** | "Proud and connected. 'I did the right thing, and I'm not alone.'" |
...
### Blueprint 6: `the_outsider`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Der Außenseiter", "en": "The Outsider"}` |
| **descriptions** | `{"de": "Kind ist anders, versucht sich anzupassen, wird am Ende für sein Anderssein geschätzt", "en": "Child is different, tries to fit in, is eventually valued for being different"}` |
| **category** | `social` |
| **min_intensity** | `deep` |
| **compatible_themes** | `{real_life, magic_fantasy, surprise}` |
| **ideal_age_groups** | `{8-9, 10-11}` |
| **compatible_learning_themes** | `{self_confidence, diversity_appreciation}` |
| **tone_guidance** | "The loneliness must be felt, not just told. The adaptation attempts should be slightly heartbreaking — the protagonist changing themselves to fit. The moment of acceptance should come from an unexpected direction." |
| **tension_curve** | "lonely → hopeful → disappointed → accepting → surprised → warm" |
| **surprise_moment** | "The thing that made them different is exactly what's needed in a crucial moment. Their 'weakness' is actually a unique strength." |
| **ending_feeling** | "I don't need to change to belong. The right people will value me for who I am." |
...
### Blueprint 7: `misunderstanding_resolved`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Das große Missverständnis", "en": "Misunderstanding Resolved"}` |
| **descriptions** | `{"de": "Freundschaft wird durch ein Missverständnis auf die Probe gestellt", "en": "Friendship tested by a misunderstanding"}` |
| **category** | `social` |
| **min_intensity** | `medium` |
| **compatible_themes** | `{real_life, adventure_action, surprise}` |
| **ideal_age_groups** | `{6-7, 8-9, 10-11}` |
| **compatible_learning_themes** | `{conflict_resolution, empathy}` |
| **tone_guidance** | "Both sides must be understandable — the reader should see HOW the misunderstanding happened. The resolution comes from LISTENING, not from one person being right." |
| **tension_curve** | "warm → confusing → angry/hurt → understanding → warmer" |
| **surprise_moment** | "When they finally talk, they discover they were both trying to do something NICE for each other — their good intentions collided." |
| **ending_feeling** | "Relief and a friendship that feels stronger because it survived a test." |
...
### Blueprint 8: `unexpected_friendship`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Unerwartete Freundschaft", "en": "Unexpected Friendship"}` |
| **descriptions** | `{"de": "Zwei sehr verschiedene Kinder entdecken eine überraschende Verbindung", "en": "Two very different children discover a surprising connection"}` |
| **category** | `social` |
| **min_intensity** | `medium` |
| **compatible_themes** | `{real_life, adventure_action, magic_fantasy, surprise}` |
| **ideal_age_groups** | `{6-7, 8-9, 10-11}` |
| **compatible_learning_themes** | `{diversity_appreciation, empathy, teamwork}` |
| **tone_guidance** | "The initial resistance should be MUTUAL — both have reasons to not like each other. The discovery of common ground should be accidental, not forced." |
| **tension_curve** | "resistant → awkward → surprised → curious → connected" |
| **surprise_moment** | "They discover they have an identical secret passion, fear, or experience that nobody else understands." |
| **ending_feeling** | "The best people are often the ones you'd never have chosen." |
...
### Blueprint 9: `letting_go`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Loslassen", "en": "Letting Go"}` |
| **descriptions** | `{"de": "Kind muss etwas/jemanden loslassen und findet Neues", "en": "Child has to let go of something/someone and finds something new"}` |
| **category** | `social` |
| **min_intensity** | `deep` |
| **compatible_themes** | `{real_life, magic_fantasy}` |
| **ideal_age_groups** | `{8-9, 10-11}` |
| **compatible_learning_themes** | `{emotional_regulation, resilience}` |
| **tone_guidance** | "Don't rush the sadness. Let it breathe. But the story should NOT be sad overall — bittersweet. The new thing doesn't REPLACE what was lost — it's different and good in its own way." |
| **tension_curve** | "clinging → resisting → sad → open → curious → bittersweet-warm" |
| **surprise_moment** | "Letting go creates space for something the protagonist couldn't have imagined. The loss enabled the gain." |
| **ending_feeling** | "It's okay to be sad about something ending AND happy about something beginning." |
...
### Blueprint 10: `first_time`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Das erste Mal", "en": "First Time"}` |
| **descriptions** | `{"de": "Kind erlebt etwas zum ersten Mal — Aufregung, Angst und Triumph", "en": "Child experiences something for the first time — excitement, fear, and triumph"}` |
| **category** | `courage` |
| **min_intensity** | `medium` |
| **compatible_themes** | `{real_life, adventure_action, surprise}` |
| **ideal_age_groups** | `{6-7, 8-9, 10-11}` |
| **compatible_learning_themes** | `{self_confidence, perseverance}` |
| **tone_guidance** | "The mix of excitement and nervousness should be physical — butterflies, wiggly legs, can't-sit-still energy. The setback should be small but feel huge. The triumph should be FELT, not just achieved." |
| **tension_curve** | "excited → nervous → setback → determination → triumph" |
| **surprise_moment** | "The hardest part isn't what they expected — the thing they prepared for was easy, but something else was the real challenge." |
| **ending_feeling** | "I did it! And I want to do it again." |
...
### Blueprint 11: `protecting_something_small`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Etwas Kleines beschützen", "en": "Protecting Something Small"}` |
| **descriptions** | `{"de": "Kind entdeckt etwas Verletzliches und riskiert etwas, um es zu beschützen", "en": "Child discovers something vulnerable and takes risks to protect it"}` |
| **category** | `courage` |
| **min_intensity** | `medium` |
| **compatible_themes** | `{magic_fantasy, adventure_action, real_life, surprise}` |
| **ideal_age_groups** | `{6-7, 8-9, 10-11}` |
| **compatible_learning_themes** | `{responsibility, empathy}` |
| **tone_guidance** | "The tenderness of caring for something small should contrast with the danger. The protagonist becomes braver than they thought possible — not for themselves, but for the thing they're protecting." |
| **tension_curve** | "tender → worried → dangerous → brave → warm" |
| **surprise_moment** | "The small thing they protected gives something back — saves them in return, reveals a gift, or simply being there was the real treasure." |
| **ending_feeling** | "Being responsible for something small made me bigger." |
...
### Blueprint 12: `doing_the_right_thing`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Das Richtige tun", "en": "Doing the Right Thing"}` |
...
| **compatible_themes** | `{real_life, adventure_action, surprise}` |
| **ideal_age_groups** | `{8-9, 10-11}` |
| **compatible_learning_themes** | `{honesty, responsibility, fairness}` |
| **tone_guidance** | "The temptation must be genuinely tempting — not obviously wrong. The inner conflict should feel like two voices arguing. The right choice should cost something real." |
| **tension_curve** | "tempted → conflicted → rationalizing → decision → consequences → peace" |
| **surprise_moment** | "Doing the right thing doesn't get immediate reward — maybe nobody even notices. But the protagonist notices the difference in themselves." |
| **ending_feeling** | "A clean conscience. The quiet satisfaction of knowing who you are when nobody's watching." |
...
## Kategorie: Empathy (Empathie)

### Blueprint 13: `walking_in_their_shoes`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "In ihren Schuhen", "en": "Walking in Their Shoes"}` |
| **descriptions** | `{"de": "Kind lernt die Perspektive einer anderen Person zu verstehen", "en": "Child learns to understand another person's perspective"}` |
| **category** | `empathy` |
| **min_intensity** | `medium` |
| **compatible_themes** | `{real_life, magic_fantasy, surprise}` |
| **ideal_age_groups** | `{8-9, 10-11}` |
| **compatible_learning_themes** | `{empathy, diversity_appreciation}` |
| **tone_guidance** | "The initial judgment should be relatable — something the reader might also think. The perspective shift should feel like an 'OH' moment, not a lecture." |
| **tension_curve** | "judgmental → curious → surprised → understanding → compassionate" |
| **surprise_moment** | "The person they judged has a reason that makes COMPLETE sense once you know it. The protagonist feels embarrassed about their earlier judgment." |
| **ending_feeling** | "Everyone has a story I don't know yet. I'll ask before I judge." |
...
### Blueprint 14: `the_invisible_helper`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Der unsichtbare Helfer", "en": "The Invisible Helper"}` |
| **descriptions** | `{"de": "Jemand hilft im Verborgenen — und löst eine Kettenreaktion der Freundlichkeit aus", "en": "Someone helps secretly — triggering a chain reaction of kindness"}` |
| **category** | `empathy` |
| **min_intensity** | `medium` |
| **compatible_themes** | `{real_life, magic_fantasy, surprise}` |
| **ideal_age_groups** | `{6-7, 8-9, 10-11}` |
| **compatible_learning_themes** | `{kindness, teamwork}` |
| **tone_guidance** | "The mystery of 'who is helping?' should drive curiosity. The reveal should be warm, not dramatic. The chain reaction of kindness should feel natural, not preachy." |
| **tension_curve** | "puzzled → curious → investigating → warm reveal → expanding warmth" |
| **surprise_moment** | "The invisible helper is someone the protagonist underestimated or didn't notice — the quiet person, the unlikely candidate." |
| **ending_feeling** | "Kindness multiplies. One person's quiet act can change everything." |
...
### Blueprint 15: `forgiving`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Vergeben", "en": "Forgiving"}` |
| **descriptions** | `{"de": "Kind wird verletzt, erlebt Wut, und findet einen Weg zu vergeben (nicht zu vergessen)", "en": "Child is hurt, experiences anger, and finds a way to forgive (not forget)"}` |
| **category** | `empathy` |
| **min_intensity** | `deep` |
| **compatible_themes** | `{real_life, surprise}` |
| **ideal_age_groups** | `{8-9, 10-11}` |
| **compatible_learning_themes** | `{emotional_regulation, conflict_resolution}` |
| **tone_guidance** | "The hurt must be legitimate — not trivial. The anger is justified. Forgiveness is NOT forgetting, NOT saying it was okay. It's choosing to let go of the weight for YOUR own sake. This is a nuanced blueprint — handle with care." |
| **tension_curve** | "hurt → angry → stuck → understanding → release → peace" |
| **surprise_moment** | "Understanding WHY someone hurt you doesn't excuse it — but it takes away its power. The person who hurt them is also hurting." |
| **ending_feeling** | "Forgiveness isn't a gift to them — it's freedom for me. I remember, but I'm not carrying it anymore." |
...
### Blueprint 16: `chaos_cascade`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Chaos-Kaskade", "en": "Chaos Cascade"}` |
| **descriptions** | `{"de": "Kleiner Fehler wird zur Lawine — totales Chaos mit befreiendem Lachen", "en": "Small mistake snowballs into total chaos with liberating laughter"}` |
| **category** | `humor` |
| **min_intensity** | `light` |
| **compatible_themes** | `{real_life, magic_fantasy, adventure_action, surprise}` |
| **ideal_age_groups** | `{6-7, 8-9, 10-11}` |
| **compatible_learning_themes** | `{}` |
| **tone_guidance** | "Each escalation should be BIGGER and MORE ABSURD than the last. The humor is in the inevitability — the reader can SEE the next disaster coming. Physical comedy, visual humor, sound effects welcome." |
| **tension_curve** | "oops → oh no → OH NO → CATASTROPHE → everything collapses → laughter" |
| **surprise_moment** | "The final result of all the chaos is accidentally BETTER than what was originally planned." |
| **ending_feeling** | "Exhausted laughter. 'I can't believe that just happened.'" |
...
### Blueprint 17: `the_plan_that_backfires`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Der Plan, der schiefgeht", "en": "The Plan That Backfires"}` |
| **descriptions** | `{"de": "Genialer Plan hat unerwartete Nebenwirkungen — es klappt anders als gedacht", "en": "Genius plan has unexpected side effects — it works out differently than planned"}` |
| **category** | `humor` |
| **min_intensity** | `light` |
| **compatible_themes** | `{adventure_action, magic_fantasy, real_life, surprise}` |
| **ideal_age_groups** | `{6-7, 8-9, 10-11}` |
| **compatible_learning_themes** | `{creative_thinking}` |
| **tone_guidance** | "The plan should sound GENIUS when explained. The backfire should be ironic — the plan works TOO well, or exactly backwards, or has a side effect nobody anticipated. The improvisation should be more fun than the original plan." |
| **tension_curve** | "scheming → confident → uh-oh → scrambling → unexpected success" |
| **surprise_moment** | "The 'failure' of the plan actually solves a DIFFERENT problem nobody knew existed." |
| **ending_feeling** | "'That was NOT the plan. But honestly? This is better.'" |
...
### Blueprint 18: `role_reversal_comedy`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Rollentausch-Komödie", "en": "Role Reversal Comedy"}` |
| **descriptions** | `{"de": "Rollen werden getauscht — komische Situationen führen zu gegenseitigem Verständnis", "en": "Roles are swapped — funny situations lead to mutual understanding"}` |
| **category** | `humor` |
| **min_intensity** | `medium` |
| **compatible_themes** | `{real_life, magic_fantasy, surprise}` |
| **ideal_age_groups** | `{6-7, 8-9, 10-11}` |
| **compatible_learning_themes** | `{empathy, responsibility}` |
| **tone_guidance** | "Both sides should be equally funny in their new roles. The humor comes from competent people being hilariously bad at something they thought was easy. The understanding at the end is EARNED through experience, not told." |
| **tension_curve** | "confident → struggling → chaotic → humbled → understanding → warm" |
| **surprise_moment** | "The thing each person found hardest about the other's role is the thing that requires the most hidden skill." |
| **ending_feeling** | "Mutual respect. 'Your job is way harder than it looks. Let's switch back.'" |
...
## Kategorie: Wonder (Staunen)

### Blueprint 19: `discovering_a_hidden_world`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Eine verborgene Welt entdecken", "en": "Discovering a Hidden World"}` |
| **descriptions** | `{"de": "Kind entdeckt eine verborgene Welt und hütet das Geheimnis", "en": "Child discovers a hidden world and guards the secret"}` |
| **category** | `wonder` |
| **min_intensity** | `medium` |
| **compatible_themes** | `{magic_fantasy, surprise, adventure_action}` |
| **ideal_age_groups** | `{6-7, 8-9, 10-11}` |
| **compatible_learning_themes** | `{curiosity, responsibility}` |
| **tone_guidance** | "The discovery should feel MAGICAL — slow, detailed, full of sensory wonder. The hidden world should have its own rules, logic, and beauty. The protagonist's awe should be palpable. Use ALL senses: what does it look like, sound like, smell like, feel like?" |
| **tension_curve** | "ordinary → curious → amazed → protective → belonging" |
| **surprise_moment** | "The hidden world has been there all along — hidden in plain sight. Once you see it, you can't unsee it." |
| **ending_feeling** | "The world is so much bigger and more magical than I thought. And I get to know the secret." |
...
### Blueprint 20: `the_impossible_made_possible`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Das Unmögliche möglich machen", "en": "The Impossible Made Possible"}` |
| **descriptions** | `{"de": "Kind stellt fest, dass das 'Unmögliche' nur 'noch nicht versucht' bedeutet", "en": "Child discovers that 'impossible' just means 'not tried yet'"}` |
| **category** | `wonder` |
| **min_intensity** | `medium` |
| **compatible_themes** | `{magic_fantasy, adventure_action, real_life, surprise}` |
| **ideal_age_groups** | `{6-7, 8-9, 10-11}` |
| **compatible_learning_themes** | `{curiosity, creative_thinking, perseverance}` |
| **tone_guidance** | "The impossibility should be stated by EVERYONE — adults, friends, books. The protagonist's curiosity should feel unstoppable. The moment of 'it works!' should be electric." |
| **tension_curve** | "told impossible → curious → experimenting → failing → adjusting → IT WORKS" |
| **surprise_moment** | "The solution was simple all along — everyone was overcomplicating it. A child's perspective sees what adults miss." |
| **ending_feeling** | "If THIS was possible... what ELSE might be? Eyes wide, mind buzzing." |
...
### Blueprint 21: `nature_speaks`

| Field | Value |
|-------|-------|
| **labels** | `{"de": "Die Natur spricht", "en": "Nature Speaks"}` |
| **descriptions** | `{"de": "Kind geht in die Natur, beobachtet etwas Erstaunliches, und sieht die Welt anders", "en": "Child goes into nature, observes something amazing, and sees the world differently"}` |
| **category** | `wonder` |
| **min_intensity** | `medium` |
| **compatible_themes** | `{real_life, magic_fantasy}` |
| **ideal_age_groups** | `{6-7, 8-9, 10-11}` |
| **compatible_learning_themes** | `{curiosity, environmental_awareness}` |
| **tone_guidance** | "SLOW DOWN. This blueprint is about NOTICING. Use all five senses. Describe nature not as background but as a character. The revelation should feel like the world is alive and has been trying to get the protagonist's attention." |
| **tension_curve** | "restless → slowing down → noticing → amazed → connected" |
| **surprise_moment** | "The natural phenomenon the protagonist witnesses explains or mirrors their own life situation in a way that changes their perspective." |
| **ending_feeling** | "The world is alive and paying attention. I just need to slow down enough to notice." |
...
| nature_speaks | ✅ | ✅ | ✅ |
| **Verfügbar pro Alter** | **13** | **21** | **21** |

> 13 Blueprints für 6-7 Jährige ist ausreichend — diese Altersgruppe bekommt häufiger `light` Intensity (kein Blueprint nötig) und die Blueprints sind weniger komplex.

---

*Dieses Dokument enthält die vollständigen Seed-Daten für die `emotion_blueprints` Tabelle. Fehlende Sprachen in `labels`/`descriptions` werden via LLM in Phase 3 generiert.*

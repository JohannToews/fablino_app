...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Avatar Builder (Mein Look): any authenticated user can read their own flag
    if (action === "getAvatarBuilderEnabled") {
      const auth = await getAuthenticatedUser(req);
      const { data: row } = await auth.supabase
        .from("app_settings")
        .select("value")
    ...
    // Avatar v2 (character_appearances, slots): any authenticated user can read their own flag
    if (action === "getAvatarV2Enabled") {
      const auth = await getAuthenticatedUser(req);
      const { data: row } = await auth.supabase
        .from("app_settings")
        .select("value")

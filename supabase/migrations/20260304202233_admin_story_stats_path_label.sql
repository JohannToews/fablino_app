-- Migration: Add story_path_code and story_path_label to admin_story_stats RPC
-- Joins story_paths table to get the human-readable label
...
RETURNS TABLE (
  ...
  story_path_code     TEXT,
  story_path_label    TEXT,
...
AS $$
  ...
  LEFT JOIN story_paths    sp ON sp.code = s.story_path_code
...
COMMENT ON FUNCTION public.admin_story_stats IS
  'Admin dashboard: per-story stats with checker metrics, ratings, path labels, and user/child info. Paginated via p_limit/p_offset.';

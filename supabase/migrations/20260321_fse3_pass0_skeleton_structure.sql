-- Migration: Pass 0 — Add skeleton structure and protagonist agency rules
--
-- Problem: Skeletons lack failed attempts, discovery moments, and protagonist
-- agency. Resolutions feel unearned because objects "teach" or protagonists
-- magically "realize" things.
--
-- Fix: Add SKELETON STRUCTURE and PROTAGONIST AGENCY rules between
-- SKELETON FORMAT RULE and QUALITY RULES.
...
SKELETON STRUCTURE (CRITICAL):
- The FIRST beat must establish the protagonist DOING something and encountering a problem.
- At least ONE beat must be a FAILED ATTEMPT — the protagonist tries and it does not work.
- The beat BEFORE the resolution must be a DISCOVERY through the protagonist's OWN action or observation — not told by an object, a book, or a narrator.
- The RESOLUTION beat must show the protagonist actively USING a setup_object as a TOOL — not receiving information from it.

PROTAGONIST AGENCY (CRITICAL):
- Every skeleton beat must have the protagonist as SUBJECT of an ACTION verb.
- BANNED verbs in skeleton: "realizes", "remembers", "feels", "senses", "understands", "knows", "learns". These are narrator shortcuts. Replace with observable actions.
- Setup objects are TOOLS the protagonist uses, not TEACHERS that deliver knowledge or visions.
...
- KNOWLEDGE CHECK: For every resolution step in the skeleton — how does the protagonist LEARN or DISCOVER how to use the setup_object or solve the problem? Point to the earlier P# where this information is established through ACTION or OBSERVATION (not told by narrator). "Realizes", "remembers", or "somehow knows" without a prior scene is NOT allowed. If the protagonist uses an object in a specific way in the resolution, an earlier beat must show them discovering that capability.
...

{
  ...
  "self_check": {
    ...
    "knowledge_sources": [
      { "resolution_step": "...", "learned_in": "P#", "how": "protagonist observed/discovered that..." }
    ]
  }
}$TMPL$,
  version = version + 1,
